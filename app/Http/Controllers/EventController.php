...
class EventController extends MyBaseController
{
    ...
    public function postCreateEvent(Request $request)
    {
        ...
        if(!empty($request->get('event_category')))
        ...
        $event->website_link = $request->get('website_link');
        $event->facebook_link = $request->get('facebook_link');
        $event->twitter_link = $request->get('twitter_link');
        $event->youtube_link = $request->get('youtube_link');
        $event->capacity = $request->get('capacity');
        $event->language = $request->get('language');
        $otherstart = $request->get('other_start_date');
        $otherend = $request->get('other_end_date');
        ...
        foreach($otherstart as $key =>$value){
            if($value)
            {
                //echo $value;
               // echo $otherend[$key];
              // echo $event->id;
             /* $neweventdate =  new EventDate;
               $neweventdate->event_id = $event->id;
              $neweventdate->start_date =date('Y-m-d h:i:s',strtotime($value));;
              $neweventdate->end_date =date('Y-m-d h:i:s',strtotime($otherend[$key]));*/
              $neweventdate['event_id'] = $event->id;
              $neweventdate['start_date'] =date('Y-m-d h:i:s',strtotime($value));;
              $neweventdate['end_date'] =date('Y-m-d h:i:s',strtotime($otherend[$key]));
              //$neweventdate->save();
              EventDate::create($neweventdate);
